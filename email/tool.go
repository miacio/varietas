...
func headerToBytes(buff *bytes.Buffer, header textproto.MIMEHeader) {
	for field, vals := range header {
		for _, subval := range vals {
			...
			case field == StrFrom || field == StrTo || field == StrCc || field == StrBcc || field == StrReplyTo:
...
func handleAddressList(v []string) []string {
	...
	for _, a := range v {
		w := strings.Split(a, ",")
		for _, addr := range w {
			decodeAddr, err := (&mime.WordDecoder{}).DecodeHeader(strings.TrimSpace(addr))
			if err == nil {
				res = append(res, decodeAddr)
			} else {
				res = append(res, addr)
			}
		}
